...
class Patchnizer:
    """
    Simple implementation of the three stages involved (extract_image_frames from video, reduce image_frames_to_patches of fixed sizes 16x16 pixels, then linearly_embed_patches into a 1D vector sequence with additional position embeddings
    """
            ...
            if file_name.endswith('.jpg'):
                ...
                for i in range(0, image.shape[0], self.patch_size):
                    for j in range(0, image.shape[1], self.patch_size):
                        ...
                        patch_path = os.path.join(frame_folder, f"patch_{count:04d}.jpg")
            ...
            if os.path.isdir(folder_path):
                ...
                for patch_name in sorted(os.listdir(folder_path), key=lambda x: int(x.split('_')[-1].split('.')[0])):
                    if patch_name.endswith('.jpg'):
                        ...
                        linear_embedding = patch.reshape(-1)
                        ...
                        embedding_path = os.path.join(embeddings_folder, f"patch_embedding_{os.path.splitext(patch_name)[0]}.txt")
                        with open(embedding_path, 'w') as f:
                            f.write(patch_embedding_text)
                        print(f"Embedding for patch {patch_name} saved.")
